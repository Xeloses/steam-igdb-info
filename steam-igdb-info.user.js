...
(function(){
    ...
    function renderApiKeyForm(){
        ...
        $J('.page_content>.rightcol.game_meta_data').prepend($J(igdbKeyForm));let $frm=$J('form#igdb_api_key_form');$frm.on('submit',(event)=>{event.preventDefault();event.stopPropagation();IGDB_API_KEY=$frm.find('input#igdb_api_key').val().trim();if(IGDB_API_KEY&&/^[a-f0-9]{32}$/.test(IGDB_API_KEY)){GM.setValue('IGDB_API_KEY',IGDB_API_KEY).then(()=>{$J('#igdb_api_key_form_container').remove();fetchGameInfo()}).catch(()=>{$frm.find('.dev_row').css('display','none');$frm.find('#igdb_api_key_error').css('display','block').text('Error attempt to save API key.')})}else{$frm.find('#igdb_api_key_error').css('display','block').text('Please, enter a valid IGDB API key.')}return false});$frm.find('#igdb_api_key_btn').on('click',()=>{$frm.submit()});
    }

    function renderGameInfo(data){
        if(data.aggregated_rating||data.rating||data.time_to_beat){let igdbData='<div><div class="block responsive_apppage_details_right heading">'+(data.url?'<a rel="noopener noreferrer" target="_blank" title="View more information on IGDB" href="'+data.url+'">IGDB</a>':'IGDB')+' game info</div></div><div class="block responsive_apppage_details_left game_details"><div class="block_content"><div class="block_content_inner"><div class="details_block">'+(data.aggregated_rating?'<div class="dev_row"><b>Critics rating:</b> '+data.aggregated_rating.toFixed(2)+'</div>':'')+(data.rating?'<div class="dev_row"><b>Players rating:</b> '+data.rating.toFixed(2)+'</div>':'');if(data.time_to_beat){igdbData+=(data.time_to_beat.normally?'<div class="dev_row"><b>Normal playthgough:</b> '+(data.time_to_beat.normally/3600).toFixed(1)+' h.</div>':'')+(data.time_to_beat.completely?'<div class="dev_row"><b>Completionist:</b> '+(data.time_to_beat.completely/3600).toFixed(1)+' h.</div>':'')+(data.time_to_beat.hastly?'<div class="dev_row"><b>Speed run:</b> '+(data.time_to_beat.hastly/3600).toFixed(1)+' h.</div>':'')}igdbData+='</div></div></div></div>';$J('.page_content>.rightcol.game_meta_data').prepend($J(igdbData))}
    }
